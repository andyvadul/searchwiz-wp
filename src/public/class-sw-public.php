...
class SearchWiz_Public {
    ...
    public function __construct() {
        ...
        // Hook into WordPress excerpt length filter to apply global setting
        add_filter( 'excerpt_length', array( $this, 'custom_excerpt_length' ), 999 );
    }

    /**
     * Apply global excerpt length setting to all WordPress excerpts.
     *
     * @param int $length Default excerpt length.
     * @return int Custom excerpt length from settings.
     */
    public function custom_excerpt_length( $length ) {
        $display_settings = get_option( 'searchwiz_display_settings', array() );
        $custom_length = isset( $display_settings['excerpt_length'] ) ? absint( $display_settings['excerpt_length'] ) : 20;

        // Debug output when searchwiz_admin_debug=1
        $debug_param = isset( $_GET['searchwiz_debug'] ) ? sanitize_text_field( wp_unslash( $_GET['searchwiz_debug'] ) ) : '';
        if ( '1' === $debug_param ) {
            error_log( sprintf(
                '[SearchWiz DEBUG] excerpt_length filter: Default=%d | Global Setting=%s | Returning=%d',
                $length,
    ...
    function wp_enqueue_styles() {
        ...
        if ( !isset( $this->opt['not_load_files']['css'] ) ) {
            wp_enqueue_style(
                'searchwiz-search-styles',
                plugins_url( '/public/css/searchwiz-search' . $min . '.css', SEARCHWIZ_PLUGIN_FILE ),
                array(),
                SEARCHWIZ_VERSION
            );

            // Add dynamic CSS from display settings
            $custom_css = $this->generate_custom_css();

            // Add customizer CSS (form-specific styles)
            $customizer_css = $this->get_customizer_css();
            if ( ! empty( $customizer_css ) ) {
                $custom_css .= "\n" . $customizer_css;
            }

            // Add menu style CSS
            $menu_css = $this->get_menu_style_css();
            if ( ! empty( $menu_css ) ) {
                $custom_css .= "\n" . $menu_css;
            }

            // Add highlight CSS - TEMPORARILY DISABLED FOR DEBUGGING
            // if ( is_search() && isset( $wp_query->query_vars['_searchwiz_settings']['highlight_terms'] ) && isset( $wp_query->query_vars['_searchwiz_settings']['highlight_color'] ) ) {
            //     $custom_css .= "\n" . '.is-highlight { background-color: ' . esc_html( $wp_query->query_vars['_searchwiz_settings']['highlight_color'] ) . ' !important;}';
            // }

            if ( ! empty( $custom_css ) ) {
                // Sanitize CSS by stripping HTML tags and escaping special characters
                $custom_css = wp_strip_all_tags( $custom_css );
                wp_add_inline_style( 'searchwiz-search-styles', $custom_css );
            }
        }
    }

    /**
     * Generate custom CSS from display settings.
     *
    ...
    function generate_custom_css() {
        ...
        // Title font size
        if ( isset( $settings['title_font_size'] ) && ! empty( $settings['title_font_size'] ) ) {
            $size = absint( $settings['title_font_size'] );
            $css .= "
            .is-ajax-search-post .is-title a,
            .searchwiz-product-card a[href] {
        ...
        // Hide thumbnails
        if ( isset( $settings['show_thumbnails'] ) && $settings['show_thumbnails'] == 0 ) {
            $css .= "
            .is-ajax-search-post .left-section,
            .searchwiz-product-card img {
        ...
        // Hide excerpts
        if ( isset( $settings['show_excerpts'] ) && $settings['show_excerpts'] == 0 ) {
            $css .= "
            .is-ajax-search-post .is-search-content,
            .is-ajax-search-post .is-description,
            .searchwiz-product-card .is-description {
                display: none !important;
            }
            ";
        }
        ...
        // Card spacing
        if ( isset( $settings['card_spacing'] ) ) {
            $spacing = absint( $settings['card_spacing'] );
            $css .= "
            .is-ajax-search-post,
            .searchwiz-product-card {
        ...
        if ( isset( $settings['border_radius'] ) ) {
            ...
            if ( $radius > 0 ) {
                ...
                .is-ajax-search-post,
                .searchwiz-product-card,
                .is-ajax-search-post .thumbnail img,
                .searchwiz-product-card img {
        ...
        // Search Box settings
        $searchbox_settings = get_option( 'searchwiz_searchbox_settings', array() );
    ...
    function wp_enqueue_scripts() {
        ...
        if ( !isset( $this->opt['not_load_files']['js'] ) ) {
            wp_enqueue_script(
                'searchwiz-search-scripts',
                plugins_url( '/public/js/searchwiz-search' . $min . '.js', SEARCHWIZ_PLUGIN_FILE ),
                array('jquery'),
                SEARCHWIZ_VERSION,
                true
            );
            ...
            if ( !$analytics_disabled ) {
                ...
                wp_localize_script( 'searchwiz-search-scripts', 'SearchWizVars', $is_temp );
            }

            wp_register_script(
                'is-highlight',
                plugins_url( '/public/js/is-highlight' . $min . '.js', SEARCHWIZ_PLUGIN_FILE ),
                array('jquery'),
                SEARCHWIZ_VERSION,
                true
            );
        ...
        if ( is_search() && isset( $wp_query->query_vars['_searchwiz_settings']['highlight_terms'] ) && 0 !== $wp_query->found_posts ) {
            $areas = array(
                '#groups-dir-list',
                '#members-dir-list',
                // BuddyPress compat
    ...
    function get_menu_search_form( $echo = true ) {
        ...
        if ( $menu_search_form ) {
            $search_form = SearchWiz_Search_Form::get_instance( $menu_search_form );
        }
        ...
        if ( isset( $this->opt['menus'] ) && isset( $this->opt['menus'][$args->theme_location] ) || isset( $this->opt['menu_name'] ) && isset( $this->opt['menu_name'][$menu_name] ) ) {
            ...
            if ( isset( $this->opt['menu_gcse'] ) && '' != $this->opt['menu_gcse'] ) {
                ...
                if ( !isset( $this->opt['menu_style'] ) || $this->opt['menu_style'] != 'default' ) {
                    if ( '' !== $title ) {
                        $link_title = ( apply_filters( 'searchwiz_show_menu_link_title', true ) ? 'title="' . esc_attr( $title ) . '"' : '' );
                        $temp .= '<a ' . $link_title . ' href="#" role="button" aria-label="' . __( "Search Title Link", "searchwiz" ) . '">';
                    } else {
                        $temp .= '<a href="#" role="button" aria-label="' . __( "Search Icon Link", "searchwiz" ) . '">';
                    }
                    if ( '' == $title ) {
                        $temp .= '<svg width="20" height="20" class="search-icon" role="img" viewBox="2 9 20 5" focusable="false" aria-label="' . __( "Search", "searchwiz" ) . '">
    ...
    function header_menu_search() {
        ...
        if ( isset( $this->opt['menu_gcse'] ) && $this->opt['menu_gcse'] != '' ) {
            ...
            if ( !isset( $this->opt['menu_style'] ) || $this->opt['menu_style'] != 'default' ) {
                $link_title = ( apply_filters( 'searchwiz_show_menu_link_title', true ) ? 'title="' . esc_attr( $title ) . '"' : '' );
                $items .= '<a ' . $link_title . ' href="#" role="button" aria-label="' . __( "Search Icon Link", "searchwiz" ) . '">';
                if ( '' == $title ) {
                    $items .= '<svg width="20" height="20" class="search-icon" role="img" viewBox="2 9 20 5" focusable="false" aria-label="' . __( "Search", "searchwiz" ) . '">
					<path class="search-icon-path" d="M15.5 14h-.79l-.28-.27C15.41 12.59 16 11.11 16 9.5 16 5.91 13.09 3 9.5 3S3 5.91 3 9.5 5.91 16 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"></path></svg>';
                } else {
                    $items .= $title;
                }
    ...
    function pre_get_posts( $query, $index_search = false ) {
        // Check if this is a SearchWiz-specific search endpoint (/sw?s=...)
        $is_searchwiz_search = get_query_var( 'is_searchwiz_search' );

        // ONLY process SearchWiz searches (/sw) or AJAX/index searches
        // Do NOT intercept regular WordPress searches (/?s=) - those are handled by redirect in template_redirect
        if ( !$is_searchwiz_search && !$index_search ) {
            return;
        }

        // If this is a SearchWiz endpoint (/sw), make it behave like a search query
        if ( $is_searchwiz_search ) {
            $query->is_search = true;
            $query->is_home = false;
            $query->is_404 = false;  // Prevent "Oops, can't find that page" flash
        }
                ...
                foreach ( $is_fields as $key => $val ) {
                    if ( isset( $val[0] ) && '' !== $val[0] ) {
                        ...
                        switch ( $key ) {
                            case '_is_includes':
                                if ( !empty( $temp ) ) {
                                    $temp = apply_filters( 'searchwiz_pre_get_posts_includes', $temp );
                            ...
                            case '_is_excludes':
                                if ( !empty( $temp ) ) {
                                    $temp = apply_filters( 'searchwiz_pre_get_posts_excludes', $temp );
                                    foreach ( $temp as $inc_key => $inc_val ) {
                                        if ( is_array( $inc_val ) && !empty( $inc_val ) || '' !== $inc_val ) {
                                            switch ( $inc_key ) {
                                                case 'post__not_in':
                                                case 'ignore_sticky_posts':
                                                    ...
                                                    if ( isset( $query->query_vars['_is_excludes']['post__not_in'] ) ) {
                                                        ...
                                                        $exclude_child = apply_filters( 'searchwiz_exclude_child', false );
                                                        if ( $exclude_child ) {
                            ...
                            case '_is_settings':
                                if ( !empty( $temp ) ) {
                                    $temp = apply_filters( 'searchwiz_pre_get_posts_settings', $temp );
                                    foreach ( $temp as $inc_key => $inc_val ) {
                                        if ( is_array( $inc_val ) && !empty( $inc_val ) || '' !== $inc_val ) {
                                            switch ( $inc_key ) {
                                                case 'posts_per_page':
        ...
        do_action( 'searchwiz_pre_get_posts', $query );
        return $query;
    }
    ...
    function posts_search( $search, $query ) {
        ...
        if ( !empty( $q['_searchwiz_settings']['search_engine'] ) && 'index' === $q['_searchwiz_settings']['search_engine'] ) {
            $is_index_search = true;
        }

        // Check if theme integration is disabled for supported themes
        // Only disable if: 1) Theme is supported AND 2) Checkbox is unchecked
        if ( !$is_index_search && $query->is_search() && $query->is_main_query() && class_exists( 'SearchWiz_Theme_Integration' ) ) {
            ...
            if ( $is_supported ) {
                ...
                if ( !$theme_integration_enabled ) {
                    return $search; // Use WordPress default search
                }
            }
            // If theme is NOT supported, always use SearchWiz (no checkbox exists)
        }
        ...
        $terms_relation_type = ( isset( $q['_searchwiz_settings']['term_rel'] ) && 'OR' === $q['_searchwiz_settings']['term_rel'] ? 'OR' : 'AND' );
        if ( isset( $this->opt['synonyms'] ) && 'OR' === $terms_relation_type ) {
            ...
            foreach ( $pairs as $pair ) {
                ...
                $value = trim( $parts[1] );
                if ( searchwiz_in_arrayi( $key, (array) $q['search_terms'] ) && !searchwiz_in_arrayi( $value, (array) $q['search_terms'] ) ) {
                    array_push( $q['search_terms'], $value );
                }
                if ( searchwiz_in_arrayi( $value, (array) $q['search_terms'] ) && !searchwiz_in_arrayi( $key, (array) $q['search_terms'] ) ) {
        ...
        if ( !isset( $q['_searchwiz_settings']['fuzzy_match'] ) ) {
            $q['_searchwiz_settings']['fuzzy_match'] = '2';
            //the default is partial fuzzy match
        }
        if ( '1' == $q['_searchwiz_settings']['fuzzy_match'] ) {
            $like = 'REGEXP';
            $f = '([[:space:][:punct:]]|^)';
            $l = '([[:space:][:punct:]]|$)';
        } else {
            if ( '2' == $q['_searchwiz_settings']['fuzzy_match'] ) {
        ...
        $search = apply_filters( 'searchwiz_posts_search_terms', $search, $q['search_terms'] );
        $search .= ")";
        ...
        $search = apply_filters( 'searchwiz_posts_search', $search );
        return $search;
    }
    ...
    function posts_join( $join, $query ) {
        ...
        $join = apply_filters( 'searchwiz_posts_join', $join );
        return $join;
    }
    ...
    function get_customizer_css() {
        ...
        foreach ( $is_form_ids as $post_id ) {
			...
			.is-form-id-<?php 
                        echo esc_attr( $post_id );
                        ?> .searchwiz-search-input::-webkit-input-placeholder {
			    color: <?php 
			...
			.is-form-id-<?php 
                        echo esc_attr( $post_id );
                        ?> .searchwiz-search-input:-moz-placeholder {
			    color: <?php 
                        echo esc_html( $search_input_color );
                        ?> !important;
			    opacity: 1;
			...
			.is-form-id-<?php 
                        echo esc_attr( $post_id );
                        ?> .searchwiz-search-input:-ms-input-placeholder {
			    color: <?php 
                        echo esc_html( $search_input_color );
                        ?> !important;
			}
     ...
     */
    function wp_head() {
        // All CSS now handled via wp_enqueue_styles() and wp_add_inline_style()
        if ( isset( $this->opt['header_search'] ) && $this->opt['header_search'] ) {
            echo do_shortcode( '[searchwiz-search id="' . esc_attr( $this->opt['header_search'] ) . '"]' );
        }

        // Inline script handling via wp_add_inline_script when JS files are not loaded
        if ( isset( $this->opt['not_load_files']['js'] ) ) {
            $is_temp_ajax = array(
                'ajaxurl'    => admin_url( 'admin-ajax.php' ),
                'ajax_nonce' => wp_create_nonce( 'searchwiz_nonce' ),
            );

            $inline_script = 'var SearchwizVars = ' . wp_json_encode( $is_temp_ajax ) . ';';
            ...
            if ( ! $analytics_disabled ) {
                ...
                if ( is_search() ) {
                    global $wp_query;
                    if ( isset( $_GET['id'] ) ) {
                        $is_temp['is_id'] = sanitize_key( wp_unslash( $_GET['id'] ) );
                    }
                    if ( isset( $_GET['s'] ) ) {
                        $is_temp['is_label'] = sanitize_text_field( wp_unslash( $_GET['s'] ) );
                    }
                    if ( 0 == $wp_query->found_posts ) {
                        $is_temp['is_cat'] = 'Nothing Found';
                    } else {
                ...
                $inline_script .= "\n" . 'var SearchWizVars = ' . wp_json_encode( $is_temp ) . ';';
            }

            // Use wp_add_inline_script instead of raw script tags
            wp_register_script( 'searchwiz-inline-vars', false, array(), SEARCHWIZ_VERSION, false );
            wp_enqueue_script( 'searchwiz-inline-vars' );
            wp_add_inline_script( 'searchwiz-inline-vars', $inline_script );
        }
    }
    ...
    function wp_footer() {
        if ( isset( $this->opt['footer_search'] ) && $this->opt['footer_search'] ) {
            echo do_shortcode( '[searchwiz-search id="' . $this->opt['footer_search'] . '"]' );
        }
        if ( isset( $this->opt['menu_style'] ) && 'popup' === $this->opt['menu_style'] ) {
            ...
            do_action( 'searchwiz_before_popup_search_form' );
            $this->get_menu_search_form();
            do_action( 'searchwiz_after_popup_search_form' );
            echo '</div></div>';
        }
    }
