...
// Define analytics function if it doesn't exist (prevents errors)
if ( !function_exists( 'searchwiz_search_analytics' ) ) {
    function searchwiz_search_analytics( $search_term, $results_count, $form_id ) {
        // Placeholder for analytics tracking
        // Can be extended later
    }
}
// Note: React mode enabled - but we still return HTML for React to parse
// The React component will parse the HTML response and extract the data
...
<?php } 
	if( $is_index_search && $index_empty ): ?>
		<div class="is-ajax-search-no-result">
			<?php esc_html_e( 'The index should be created for the search engine to work.', 'searchwiz' ); ?>
		</div>		
		<?php 
		return;
	endif;

	if ( isset( $is_includes['post_type'] ) && in_array( 'product', $is_includes['post_type']  ) && ! SearchWiz_Help::is_woocommerce_inactive() ) {
	...
	$post_args = apply_filters( 'searchwiz_ajax_search_args', $post_args );
	$posts = new WP_Query( $post_args );
	do_action( 'searchwiz_ajax_search_results', $posts );
	if ( $posts->posts ) {
	    ...
	    foreach ( $posts->posts as $post ) :
	        ...
	        $product = '';
	        $product_class = '';
	        if( ( 'product' === $post->post_type || 'product_variation' === $post->post_type ) && function_exists( 'wc_get_product' ) && ! SearchWiz_Help::is_woocommerce_inactive() ) {
		...
		?>
		<div class="is-ajax-search-no-result">
			<?php echo esc_html(html_entity_decode( $field['nothing_found_text'] )); ?>
		</div>
		<?php
	}
	?>
...
<?php } ?>
	    ...
	    ?>
	    	<div class="is-show-more-results <?php echo esc_attr( $show_more_class );?>" data-page="<?php echo esc_attr($next_page); ?>">
		    	...
		    	// AAJX Loader.
                        ...
                        if( $loader_image ) {
                                echo '<img class="is-load-more-image" alt="'. esc_attr__( "Loader Image", 'searchwiz' ) .'" src="'.esc_attr( $loader_image ).'" style="display: none;" />';
                        }
                    }
		    	?>
	    	</div>
...
<?php if( isset( $field['show_details_box'] ) && $field['show_details_box'] ) { ?>
    <div id="is-ajax-search-details-<?php echo esc_attr( $search_post_id ); ?>" class="is-ajax-search-details">
