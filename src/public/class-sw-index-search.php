...
class SearchWiz_Index_Search {
	...
	public function __construct() {
		...
		$this->helper = new SearchWiz_Index_Helper( $index_opt );
		$this->model  = new SearchWiz_Index_Model();
	}
	...
	public function posts_pre_query( $posts, $wp_query ) {
		...
		if ( $wp_query->is_search()
			&& ( $wp_query->is_main_query() || check_ajax_referer( 'searchwiz_nonce', 'security', false ) )
	 ...
	 * Get posts.
	 *
	 * Use SearchWiz_Public's pre_get_posts method to restrict the query.
	 *
	 * @since 1.0.0
	 *
	...
	protected function get_posts( $post_ids, $s ) {
		...
		if ( ! empty( $post_ids ) ) {
			...
			$orderby     = $search_form->group_prop( '_searchwiz_settings', 'orderby' );
			$order       = $search_form->group_prop( '_searchwiz_settings', 'order' );

			$ignore_sticky_posts = $search_form->group_prop( '_is_excludes', 'ignore_sticky_posts' );
			$move_sticky_posts   = $search_form->group_prop( '_searchwiz_settings', 'move_sticky_posts' );
			...
			do {
				...
				if ( ! empty( $slice_ids ) ) {
					...
					$is_public = SearchWiz_Public::getInstance();
	...
	public function search( $s ) {
		...
		if ( ! empty( $search_terms ) ) {
			...
			$fuzzy_match = $search_form->group_prop( '_searchwiz_settings', 'fuzzy_match' );
			...
			$all_matches = new SearchWiz_Index_Matches( $search_form );
		...
		return apply_filters( 'searchwiz_index_search_results', $post_ids, $search_terms, $all_matches );
	}
	 ...
	 * @param SearchWiz_Index_Matches $all_matches The object containing all matches found.
	 * @return array The ranked post_ids found for the search.
	 */
	public function rank( $search_terms, $orig_search_terms, $all_matches ) {
		...
		$search_rel  = $this->search_form->group_prop( '_searchwiz_settings', 'term_rel', 'AND' );
		$fuzzy_match = $this->search_form->group_prop( '_searchwiz_settings', 'fuzzy_match', 2 );
	...
	public function add_stemms( $search_terms, $force = false ) {
			if ( class_exists( 'SW_Stemmer' ) ) {
				$keyword_stem = $this->search_form->group_prop( '_searchwiz_settings', 'keyword_stem' );
				if ( ! empty( $keyword_stem ) ) {
					foreach ( $search_terms as $term ) {
						$stem = SW_Stemmer::Stem( $term );
	 ...
	 * @since 1.0.0
	 *
	 * @return SearchWiz_Search_Form The retrieved object.
	 */
	public function load_search_form() {

		$this->search_form = SearchWiz_Search_Form::load_from_request();
	 ...
	 * @param bool $force Force reloading from DB.
	 * @return SW_Option The retrieved object.
	 */
