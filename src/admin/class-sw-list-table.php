...
class SearchWiz_List_Table extends WP_List_Table {

	public static function define_columns() {
		$columns = array(
			'cb'		=> '<input type="checkbox" />',
			'title'		=> __( 'Title', 'searchwiz' ),
			'shortcode' => __( 'Shortcode', 'searchwiz' ),
			'date'		=> __( 'Date', 'searchwiz' ),
		);
	...
	function prepare_items() {
		...
		$this->items = SearchWiz_Search_Form::find( $args );

		$total_items = SearchWiz_Search_Form::count();
		$total_pages = ceil( $total_items / $per_page );

                if ( 1 == $total_items && ! isset( $_GET['s'] )  ) {
                    if ( isset( $this->items[0] ) && $this->items[0]->id() ) {
                        $redirect_to = menu_page_url( 'searchwiz-search', false ) . '&post='.absint( $this->items[0]->id() ).'&action=edit';
	...
	function column_title( $item ) {
		$url = admin_url( 'admin.php?page=searchwiz-search&post=' . absint( $item->id() ) );
		...
		$output = sprintf(
			...
			esc_attr( sprintf( __( 'Edit &#8220;%s&#8221;', 'searchwiz' ),
				$item->title() ) ),
			esc_html( $item->title() )
		);
		...
		if ( current_user_can( 'is_edit_search_form', $item->id() ) ) {
			...
			$actions = array_merge( $actions, array(
				'copy' => sprintf( '<a href="%1$s">%2$s</a>',
					esc_url( $copy_link ),
					esc_html( __( 'Duplicate', 'searchwiz' ) )
				),
			) );
		}
		...
		if ( current_user_can( 'is_delete_search_form', $item->id() ) && ( 'default-search-form' !== $item->name() || defined( 'DELETE_DEFAULT_SEARCH_FORM' ) ) ) {
			...
			$actions = array_merge( $actions, array(
				'delete' => sprintf( '<a href="%1$s" %2$s>%3$s</a>',
					esc_url( $delete_link ),
					"onclick=\"if (confirm('" . esc_js( __( "You are about to delete this search form.\n  'Cancel' to stop, 'OK' to delete.", 'searchwiz' ) ) . "')) {return true;} return false;\"",
					esc_html( __( 'Delete', 'searchwiz' ) )
				),
			) );
		}
	...
	function column_gmdate( $item ) {
		...
		$t_time = mysql2gmdate( __( 'Y/m/d g:i:s A', 'searchwiz' ),
			$post->post_date, true );
		$m_time = $post->post_date;
		$time = mysql2gmdate( 'G', $post->post_date )
		...
		if ( $time_diff > 0 && $time_diff < 24*60*60 ) {
			...
			$h_time = sprintf(
				...
				__( '%s ago', 'searchwiz' ), human_time_diff( $time ) );
		} else {
			$h_time = mysql2gmdate( __( 'd/m/Y', 'searchwiz' ), $m_time );
		}
