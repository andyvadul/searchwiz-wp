...
class SearchWiz_Index_Builder {
	 ...
	 *
	 * @since 1.0.0
	 * @param SearchWiz_Index_Options $index_opt The plugin index options.
	 * @param SearchWiz_Index_Helper  $helper The index helper object.
	 * @param SearchWiz_Index_Model   $model The index model object.
	 */
	...
	public function index_post( $post, $remove_first = false ) {
		...
		if ( ! $post instanceof WP_Post ) {
			throw new Exception( esc_html( 'Post not found', 'searchwiz' ) );
		}
	...
	protected function save_indexes() {
		 ...
		 *
		 * @since 1.0.0
		 * @param SearchWiz_Index_Model[] The indexes to save.
		 * @param self $this The reference to this object.
		 */
		$this->indexes = apply_filters( 'searchwiz_save_indexes', $this->indexes, $this );
	...
	protected function is_indexable_post() {
		...
		if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) || 'auto-draft' === $this->post->post_status ) {
			...
			if ( empty( $post_type ) || ! in_array( $post_type, $post_types ) ) {
				$indexable       = false;
				$no_index_reason = __( 'Post is not in selected post types', 'searchwiz' );
			}

			// Check if this is a Jetpack Contact Form entry.
			if ( isset( $_REQUEST['contact-form-id'] ) && ! empty( sanitize_text_field( wp_unslash( $_REQUEST['contact-form-id'] ) ) ) ) {
				$indexable       = false;
				$no_index_reason = __( 'Post is a Jetpack contact form', 'searchwiz' );
			}
			...
			if ( is_plugin_active( 'wordpress-seo/wp-seo.php' ) ) {
				...
				if ( $indexable && ( !empty( $noindex ) && 1 == $noindex && empty( $yoast_opt) ) ) {
					$indexable       = false;
					$no_index_reason = __( 'Post marked as Yoast no index', 'searchwiz' );
				}
			}
		}
	...
	protected function index_post_content() {
		 ...
		 */
		$to_index = apply_filters( 'searchwiz_index_post_content', $to_index, $this->post );
	...
	protected function get_meta_fields() {
		 ...
		 */
		return apply_filters( 'searchwiz_index_get_meta_fields', $to_index, $this->post );
	}
	...
	protected function get_terms() {
		...
		switch ( $opt->tax_index_opt ) {
			case SearchWiz_Index_Options::TAX_OPT_ALL:
			...
			case SearchWiz_Index_Options::TAX_OPT_SELECT:
		 ...
		 * @param WP_Post The post to get the terms for.
		 */
		return apply_filters( 'searchwiz_index_get_terms', $terms, $this->post );
	}
	...
	protected function get_comments( $args = array() ) {
		 ...
		 */
		$to_index = apply_filters( 'searchwiz_index_get_comments', $to_index, $this->post );
	...
	protected function index_attachment() {
		if ( 'attachment' == $this->get_post_type() ) {
			 ...
			 */
			do_action( 'searchwiz_index_attachment', $attachment, $mime_type, $this );
	 ...
	 * @return SearchWiz_Index_Model The token index object.
	 */
	protected function get_index( $token ) {
		if ( ! empty( $this->indexes[ $token ] ) ) {
			 ...
			 *
			 * @since 1.0.0
			 * @param SearchWiz_Index_Model $index The index for a token.
			 * @param string $token The token.
			 * @param WP_Post $post The respective post.
			 */
			$index                   = apply_filters( 'searchwiz_get_index', $index, $token, $this->post );
