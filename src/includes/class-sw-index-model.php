...
class SearchWiz_Index_Model {
	 ...
	 * @since 1.0.0
	 */
	const SW_INDEX_TABLE = 'is_inverted_index';
	 ...
	 * @since 1.0.0
	 */
	const SW_DB_VER_OPT = 'is_db_version';
	...
	public static function get_is_index_table_name() {
		global $wpdb;
		$is_index_table = $wpdb->prefix . self::SW_INDEX_TABLE;
	...
	public static function verify_is_index_table_exists( $create_if_not_exists = false ) {
		...
		if ( $wpdb->get_var( $wpdb->prepare( "SHOW TABLES LIKE %s", $is_index_table ) ) === $is_index_table ) {
	...
	protected static function index_exists( $tbl_index ) {
		...
		if ( empty( $tbl_indexes ) ) {
			$is_index_table = self::get_is_index_table_name();
			$sql = "SHOW INDEX FROM " . esc_sql( $is_index_table );
			$tbl_indexes    = $wpdb->get_results( $sql ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		}
	...
	public static function uninstall() {
		...
		delete_option( self::SW_DB_VER_OPT );
	}
	...
	public static function delete_tables() {
		...
		if ( self::verify_is_index_table_exists() ) {
			$sql = "DROP TABLE " . esc_sql( $is_index_table );
	...
	public static function clear_index_table() {
		...
		if ( self::verify_is_index_table_exists() ) {
			$sql = "TRUNCATE TABLE " . esc_sql( $is_index_table );
			return $wpdb->query( $sql ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared		
		}
	}
	...
	public static function count_indexed_posts() {
		...
		if ( self::verify_is_index_table_exists() ) {
			$sql = "SELECT COUNT(DISTINCT(post_id)) FROM " . esc_sql( $is_index_table );
	...
	public static function count_indexed_terms() {
		...
		if ( self::verify_is_index_table_exists() ) {
			$sql = "SELECT COUNT(DISTINCT(term)) FROM " . esc_sql( $is_index_table );
			$count = $wpdb->get_var( $sql ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		}
	...
	public static function count_index_size() {
		...
		if ( self::verify_is_index_table_exists() ) {
			$sql = "SELECT COUNT(*) FROM " . esc_sql( $is_index_table );
			$count = $wpdb->get_var( $sql ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		}
	...
	public function calc_score( $weights = null ) {
		...
		$weights = apply_filters( 'searchwiz_index_calc_score_weights', $weights );
	 ...
	 *
	 * @since 1.0.0
	 * @param array <SearchWiz_Index_Model> The array of objects to save.
	 * @return int The number of inserted rows.
	 */
	public static function save_indexes( $rows ) {
		...
		if ( ! empty( $rows ) ) {
			foreach ( $rows as $row ) {
				...
				$sql .= $wpdb->prepare(
					$placeholders, // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
					$row->post_id,
					$term,
					$reverse,
					$row->score,
					...
					$row->taxonomy,
					$row->customfield,
					! empty( $row->taxonomy_detail ) ? wp_json_encode( $row->taxonomy_detail ) : '',
					! empty( $row->customfield_detail ) ? wp_json_encode( $row->customfield_detail ) : '',
					$row->type,
					$row->lang
				);
			}
			...
			$rows_qty = $wpdb->query( $sql ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
			if ( empty( $rows_qty ) && $wpdb->last_error ) {
				$msg = __( 'Could not save index rows', 'searchwiz' );
	...
	public static function delete_post_index( $post_id ) {
		global $wpdb;
		$table_name = self::get_is_index_table_name();
		$sql = $wpdb->prepare(
			"DELETE FROM {$table_name} WHERE post_id = %d",
			$post_id
		);
		$ret = $wpdb->query($sql); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	 ...
	 * @return array <SearchWiz_Index_Model> The array of objects to save.
	 */
	...
	public static function search( $args ) {
		...
		$sql = $wpdb->prepare(
			"SELECT * FROM {$is_index_table} i WHERE type IN ( {$types} )"
		);
	...
	public static function get_db_version() {
		return get_option( self::SW_DB_VER_OPT );
	}
	...
	public static function save_db_version( $db_version ) {
		...
		return update_option( self::SW_DB_VER_OPT, $db_version );
	}
