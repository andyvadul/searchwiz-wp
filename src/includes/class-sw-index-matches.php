...
class SearchWiz_Index_Matches {
		...
		foreach ( $matches as $term => $match ) {
			foreach ( $match as $post_id => $index ) {
				if ( empty( $this->matches[ $post_id ] ) ) {
					$this->matches[ $post_id ] = new SearchWiz_Index_Match( $post_id );
				}
	...
	protected function filter_matches( $all_matches ) {
		...
		return apply_filters( 'searchwiz_index_get_terms_counts', $all_matches );
	}
	...
	public function calc_match_points( $orig_search_terms, $index_size ) {
		...
		return apply_filters(
			'searchwiz_index_calc_match_points',
			$match_points,
			$orig_search_terms
		);
	}
