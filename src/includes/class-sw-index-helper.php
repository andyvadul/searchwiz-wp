...
class SearchWiz_Index_Helper {
	 ...
	 *
	 * @since 1.0.0
	 * @param SW_Index_Option $index_opt The index options.
	 */
	...
	public function block_rendering( $content ) {
		...
		foreach ( $blocks as $block ) {
			 ...
			 */
			$block = apply_filters( 'searchwiz_block_to_render', $block );
			...
			if (
				 ...
				 */
				$output .= apply_filters( 'searchwiz_rendered_block', render_block( $block ), $block );
			}
		}
	...
	public function disable_problematic_shortcodes() {
		$disable_shortcodes = apply_filters(
			'searchwiz_disable_shortcodes_array',
			array(
	...
	public function remove_page_builder_shortcodes( $content ) {
		...
		$search_array = apply_filters(
			'searchwiz_page_builder_shortcodes',
			array(
				// Remove content.
	...
	public function remove_punctuation( $a ) {
		...
		$endash_replacement = ' ';
		$emdash_replacement = ' ';
		if ( SearchWiz_Index_Options::PUNC_OPT_REMOVE == $this->index_opt->hyphens ) {
			$hyphen_replacement = '';
			$endash_replacement = '';
			$emdash_replacement = '';
		}
		if ( SearchWiz_Index_Options::PUNC_OPT_KEEP == $this->index_opt->hyphens ) {
		...
		$quote_replacement = ' ';
		if ( SearchWiz_Index_Options::PUNC_OPT_REMOVE == $this->index_opt->quotes ) {
			$quote_replacement = '';
		}

		$ampersand_replacement = ' ';
		if ( SearchWiz_Index_Options::PUNC_OPT_REMOVE == $this->index_opt->ampersands ) {
			$ampersand_replacement = '';
		}
		if ( SearchWiz_Index_Options::PUNC_OPT_KEEP == $this->index_opt->ampersands ) {
			$ampersand_replacement = 'AMPERSANDTAIKASANA';
		}

		$decimal_replacement = ' ';
		if ( SearchWiz_Index_Options::PUNC_OPT_REMOVE == $this->index_opt->decimals ) {
			$decimal_replacement = '';
		}
		if ( SearchWiz_Index_Options::PUNC_OPT_KEEP == $this->index_opt->decimals ) {
	...
	public function strip_tags( $content ) {
		...
		return strip_tags(
			$content,
			apply_filters( 'searchwiz_allowable_tags', '' )
		);
	}
