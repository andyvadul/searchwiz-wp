...
class SearchWiz_Index_Manager extends SearchWiz_Base_Options {
	 ...
	 * @since 1.0.0
	 */
	const CREATE_INDEX_ACTION = 'searchwiz_create_index';
	const DELETE_INDEX_ACTION = 'searchwiz_delete_index';
	const INDEX_POST_ACTION   = 'searchwiz_index_post';
	...
	public function __construct() {
		...
		$helper        = new SearchWiz_Index_Helper( $index_opt );
		$model         = new SearchWiz_Index_Model();
		$this->model   = $model;
		$this->builder = new SearchWiz_Index_Builder( $index_opt, $helper, $model );
	...
	public function process_actions() {
		...
		if ( self::RESET_ACTION == $action ) {
			...
			if ( ! check_admin_referer( self::RESET_ACTION )
				...
				wp_die( esc_html__( 'You are not allowed to reset these configurations.', 'searchwiz' ) );
			}
			...
			$settings_updated = isset( $_REQUEST['settings-updated'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['settings-updated'] ) ) : '';
			if ( ! empty( $request_message )
				&& 'index-reset' == $request_message
				&& ! empty( $settings_updated )
			) {
				unset( $_GET['message'] );
			}
		}
	...
	public function get_index_status( $only_status = false, $results_lbl = '' ) {
		$total_posts = $this->count_posts_to_index();
		$indexed_posts = $this->model->count_indexed_posts();
		$remaining_posts = max( 0, $total_posts - $indexed_posts );

		// Format last indexed date
		$last_indexed_text = __( 'Never indexed', 'searchwiz' );
		if ( ! empty( $this->build_end_time ) ) {
			// Convert UTC timestamp to site's local time
			// get_date_from_gmt() converts from UTC to site timezone
			$gmt_date = gmdate( 'Y-m-d H:i:s', $this->build_end_time );
			$local_date = get_date_from_gmt( $gmt_date, get_option( 'date_format' ) . ' ' . get_option( 'time_format' ) );
			$last_indexed_text = sprintf(
				/* translators: %s: date and time of last indexing */
				__( 'Last indexed: %s', 'searchwiz' ),
				$local_date
			);
		}

		$results = array(
			__( 'Content Status', 'searchwiz' ),
			sprintf(
				'%d %s',
				$total_posts,
				__( 'total contents available', 'searchwiz' )
			),
			$last_indexed_text,
			null,
			__( 'Index Status', 'searchwiz' ),
			sprintf(
				'%d %s',
				$indexed_posts,
				__( 'unique content in the index', 'searchwiz' )
			),
			sprintf(
				'%d %s',
				$this->model->count_indexed_terms(),
				__( 'unique terms in the index', 'searchwiz' )
			),
			sprintf(
				'%d %s',
				$this->model->count_index_size(),
				__( 'index records', 'searchwiz' )
			),
			sprintf(
				'%d %s',
				$remaining_posts,
				__( 'contents remaining to index', 'searchwiz' )
			),
			null,
		);
		...
		if ( ! $only_status ) {
			...
			if ( ! empty( $build_results ) && is_array( $build_results ) ) {
				if ( $results_lbl ) {
					...
					$results[] = __( 'Last build results:', 'searchwiz' );
				}
	...
	public function get_ajax_create_index_data() {
		...
		return array(
			...
			'indexed'    => $indexed,
			'total'      => $total,
			'start_msg'  => __( 'Creating Index...', 'searchwiz' ),
			'btn_labels' => array(
				self::IDX_EMPTY    => __( 'Create Index', 'searchwiz' ),
				self::IDX_CREATING => __( 'Pause Creating Index', 'searchwiz' ),
				self::IDX_PAUSING  => __( 'Pausing Creating Index', 'searchwiz' ),
				self::IDX_PAUSED   => __( 'Resume Creating Index', 'searchwiz' ),
				self::IDX_CREATED  => __( 'Recreate Index', 'searchwiz' ),
			),
	...
	public function ajax_create_index() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			esc_html( "Error: You don't have permission to create index. ", "searchwiz" );
			wp_die();
		}
		...
		$nonce = isset( $_POST['_isnonce'] ) ? sanitize_text_field( wp_unslash( $_POST['_isnonce'] ) ) : '';
		if ( wp_verify_nonce( $nonce, self::CREATE_INDEX_ACTION )
			...
			$results               = sprintf(
				'- %s %d %s (%d / %d), %d %s, %d %s',
				__( 'indexed', 'searchwiz' ),
				$indexed - $this->build_offset,
				__( 'contents', 'searchwiz' ),
				$indexed,
				$total,
				$terms,
				__( 'terms', 'searchwiz' ),
				$exec_time,
				__( 'seconds', 'searchwiz' )
			);
			$this->build_results[] = $results;

			$page++;
			$this->build_offset = $indexed;
			$results = $this->get_index_status( false, __( 'Creating Index...', 'searchwiz' ) );

			if ( $this->build_offset >= $total ) {
				$page               = 1;
				$finished_msg = __( 'Finished Creating Index.', 'searchwiz' );
			...
			$results = __( 'Error: Index is already executing. Try again in a few minutes.', 'searchwiz' );
		} else {
			$results = __( 'Error: Index was not created', 'searchwiz' );
		}
	...
	protected function get_query_args( $args = array() ) {
		...
		return apply_filters( 'searchwiz_get_posts_to_index_args', $args );
	...
	protected function get_build_pagination_args( $page ) {
		...
		$max_per_page  = apply_filters( 'searchwiz_index_max_per_page', 100 );
		$multiplier    = 2;
	...
	public function get_ajax_delete_index_data() {
		return array(
			...
			'start_msg' => esc_html__( 'Deleting Index...', 'searchwiz' ),
		);
	}
	...
	public function ajax_delete_index() {
		$results = '';
		$nonce = isset( $_POST['_isnonce'] ) ? sanitize_text_field( wp_unslash( $_POST['_isnonce'] ) ) : '';
		if ( wp_verify_nonce( $nonce, self::DELETE_INDEX_ACTION ) ) {
			$this->delete_index();
			$results = __( 'Index deleted', 'searchwiz' );
		} else {
			$results = __( 'Error: Index was not deleted', 'searchwiz' );
		}
		$results .= PHP_EOL . PHP_EOL . $this->get_index_status( true );
		$ret      = array(
			'results'   => esc_textarea( $results ),
			'btn_label' => esc_html__( 'Create Index', 'searchwiz' ),
		);
	...
	public function get_ajax_index_post_data() {
		return array(
			...
			'post_id'   => 0,
			'start_msg' => esc_html__( 'Indexing content...', 'searchwiz' ),
		);
	}
	...
	public function ajax_index_post() {
		...
		$results = '';
		$nonce = isset( $_POST['_isnonce'] ) ? sanitize_text_field( wp_unslash( $_POST['_isnonce'] ) ) : '';
		if ( $post_id && wp_verify_nonce( $nonce, self::INDEX_POST_ACTION ) ) {

			ob_start();

			if ( $this->index_post( $post_id ) ) {
				$results = esc_html__( 'Content Indexed ID: ', 'searchwiz' ) . $post_id;
				$error   = false;
			}
		...
		if ( $error ) {
			$results = esc_html__( 'Error: content was not indexed', 'searchwiz' );
